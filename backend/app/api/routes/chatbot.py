...
def get_sessions(
    ...
    return chat_service.get_sessions(db, user_id=current_user.id, skip=skip, limit=limit)
...
def add_message(
    ...
    session = chat_service.get_session(db, message_in.session_id)
    if not session:
        raise HTTPException(status_code=404, detail="Chat session not found")
    if session.user_id and session.user_id != current_user.id:
        raise HTTPException(status_code=403, detail="Forbidden")
...
def ask_ai(
    ...
    # Ensure session belongs to current_user
    if session.user_id and session.user_id != current_user.id:
        raise HTTPException(status_code=403, detail="Forbidden")
