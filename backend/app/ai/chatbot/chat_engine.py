...
class ChatEngine:
    ...
    def get_response(
        ...
        Retries up to max_retries times on 429 rate-limit errors using
        exponential backoff with jitter.
        Returns None on any non-retriable error so callers can fall back gracefully.
        """
        if not self.api_key:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if "429" in exc_str and attempt < max_retries:
                    ...
                    logger.warning(
                        "ChatEngine: 429 rate-limit on attempt %d/%d. Retrying in %.1fs...",
